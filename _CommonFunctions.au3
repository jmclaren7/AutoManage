...
;===============================================================================
Func _RunWait($sProgram, $Working="")
	Local $sData, $sStdOut, $iPid
	$iPid=Run($sProgram, $Working, @SW_HIDE, $STDERR_MERGED)
	If @error then
...
; Description:		ProcessWaitClose that handles stdout from the running proccess
; Call With:		_ProcessWaitClose($iPid)
; Parameter(s):
; Return Value(s):  On Success -
; 					On Failure -
; Author(s):        JohnMC - www.TeamMC.cc
; Date/Version:		06/02/2014  --  v1.0
;===============================================================================
Func _ProcessWaitClose($iPid)
	...
	While ProcessExists($iPid)
		...
		$sStdOut = StdoutRead($iPid)
		If $sStdOut = "" Then ContinueLoop
		$sStdOut=StringReplace($sStdOut,@CR&@LF,"")
		_ConsoleWrite($sStdOut)
		$sData &= $sStdOut
	WEnd
	return $sData
...
Func _ConsoleWrite($sMessage, $iLevel=1, $iSameLine=0)
	...
	if Eval("LogFilePath")="" Then Local $LogFilePath = StringTrimRight(@ScriptFullPath,4)&"_Log.txt"
	if Eval("LogFileMaxSize")="" Then Local $LogFileMaxSize = 0
	if Eval("LogToFile")="" Then Local $LogToFile = False
	if Eval("LogLevel")="" Then Local $LogLevel = 3 ; The level of message to log - If no level set to 3
